"""Opereta property listing adapter.
...
class OperetaScraper(BaseScraper):
    """Collect all supported property types from Opereta and emit normalized DTOs."""
    ...
    def scrape(self) -> list[PropertyDTO]:
        ...
        LOGGER.info("Found %d Opereta property detail links", len(links))
    ...
    def _collect_links(self) -> list[str]:
        links: set[str] = set()
        first_url = self._working_listing_url()
        if first_url == "sitemap://opereta":
            return list(self._sitemap_links)
        # Opereta exposes property-type landing pages as well as numbered result
        # pages. Walk both rather than following only rel=next (which left the
        # database with the handful of cards visible on the first page).
        pending = [first_url]
        visited: set[str] = set()
        while pending and len(visited) < self.max_pages:
            ...
            response = self.http.get(page_url, timeout=self.timeout)
            response.raise_for_status()
            ...
            for anchor in soup.select("a[href]"):
                ...
                if self._is_property_link(absolute, anchor):
                    links.add(absolute)
                elif self._is_results_page_link(absolute, anchor) and absolute not in visited:
                    pending.append(absolute)
        if links:
            LOGGER.info("Scanned %d Opereta result pages", len(visited))
            return sorted(links)
        sitemap_links = self._links_from_sitemaps()
        if sitemap_links:
            return sitemap_links
        raise requests.RequestException(
            f"Opereta stranica {self.listing_url} nije sadržavala poveznice na oglase."
        )
    ...
    def _is_property_url(url: str) -> bool:
        ...
        if excluded_segments.intersection(path_parts):
            return False
        # Detail URLs use the singular segment and contain a listing identifier.
        # Plural /nekretnine/... URLs are result/category pages, not properties.
        return bool(re.search(
            r"/(?:nekretnina|property)/(?:[^/?#]+/)?(?:[^/?#]*\d{3,}[^/?#]*)/?$",
            urlparse(lowered).path,
        ))
    ...
    def _is_property_link(self, url: str, anchor: Tag) -> bool:
        if urlparse(url).netloc != urlparse(self.base_url).netloc:
            return False
        return self._is_property_url(url)

    def _is_results_page_link(self, url: str, anchor: Tag) -> bool:
        """Identify category and pagination links belonging to property search."""
        parsed = urlparse(url)
        if parsed.netloc != urlparse(self.base_url).netloc:
            return False
        path = parsed.path.lower().rstrip("/")
        if not (path.endswith("/nekretnine") or "/nekretnine/" in f"{path}/"):
            return False
        classes = " ".join(str(value) for value in (anchor.get("class") or []))
        rel = " ".join(str(value) for value in (anchor.get("rel") or []))
        label = str(anchor.get("aria-label") or "")
        is_pagination = bool(re.search(r"(?:^|[?&])(page|paged|stranica)=\d+", parsed.query)) or bool(
            re.search(r"/(?:page|stranica)/\d+/?$", path)
        ) or any(token in f"{classes} {rel} {label}".lower() for token in ("next", "pagination", "sljede"))
        # Type/category landing pages are needed to discover every listing type.
        is_category = len([part for part in path.split("/") if part]) >= 2 and not parsed.query
        return is_pagination or is_category
    ...
    def parse_detail(cls, html: str, url: str) -> PropertyDTO:
        ...
        property_type = cls._property_type(url, soup, str(title))
        return PropertyDTO(
            portal="opereta", portal_id=str(identifier), url=url, title=str(title), property_type=property_type,
    ...
    @staticmethod
    def _property_type(url: str, soup: BeautifulSoup, title: str) -> str:
        """Map Opereta's Croatian/English labels to stable application values."""
        context = " ".join(
            [urlparse(url).path, title, *(node.get_text(" ", strip=True) for node in soup.select(
                ".breadcrumb, .breadcrumbs, [class*='property-type']"
            ))]
        ).lower()
        mappings = (
            ("apartment", r"\b(stan(?:ovi|a|u)?|apartment|flat)\b"),
            ("house", r"\b(kuć[aeu]?|kuca|house|villa|vila)\b"),
            ("land", r"\b(zemljišt\w*|zemljist\w*|land|plot)\b"),
            ("commercial", r"\b(poslovn\w*|commercial|office|ured|lokal)\b"),
            ("vacation", r"\b(odmor|vacation|holiday)\b"),
        )
        return next((value for value, pattern in mappings if re.search(pattern, context)), "other")
